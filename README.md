...
4. The widget stores the pending run in `localStorage` (key `ff_pending_run`) and polls `<CHAT_BACKEND_URL>/api/poll?p=...` **every 3 seconds**. Each call is forwarded server-side to the agent by `poll.mjs`, which validates that the target stays on `AGENT_WEBHOOK_URL`'s origin (SSRF guard).
