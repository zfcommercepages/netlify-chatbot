...
1. The widget POSTs `{ prompt, context }` to `/api/chat`. `context` is a JSON array of up to 10 `{ user_prompt, agent_response }` pairs maintained by the widget in `localStorage` (key `ff_history`). Every conversational turn is recorded — greetings, category/collection/cart views, product searches, and agent answers — so the agent always sees the full local history.
